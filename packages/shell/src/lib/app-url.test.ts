...
describe('parseAppUrl / buildAppUrl', () => {
  ...
  it('round-trips playground JSON graph URL with query', () => {
    const state = parseAppUrl({
      appPath: '/playground/json/wireframe/graph',
...
describe('screenPathForDoc', () => {
  ...
  it('builds playground JSON screen path', () => {
    expect(screenPathForDoc('playground', 'json', 'wireframe', 'home')).toBe(
      '/playground/json/wireframe/home',
    )
  })
...
describe('resolveLegacyAppPath', () => {
  ...
  it('maps /playground/home to playground JSON prototype', () => {
    expect(
      resolveLegacyAppPath('/playground/home', [{ slug: 'wireframe', screenIds: ['home'] }]),
    ).toEqual({
      app: 'playground',
      source: 'json',
      docSlug: 'wireframe',
